...
export default function SiteVisit() {
  ...
  function downloadOdOpReport() {
    ...
    const hdrs = ["#","Person","Nominal (Site ID)","Site Name","Type of Site","Incident Remark","Problem Resolved","Time/Date","GPS Status","GPS Distance","Employee GPS","Master GPS"];
    const data = activeReport.odOpRows.map((r, i) => [
      ...
      r.closestPing ? `${r.closestPing.lat.toFixed(5)}, ${r.closestPing.lng.toFixed(5)}` : "",
      r.siteLat != null ? `${r.siteLat.toFixed(5)}, ${r.siteLng.toFixed(5)}` : "",
    ]);
    const xlWs = XLSX.utils.aoa_to_sheet([hdrs, ...data]);
    xlWs["!cols"] = [5,20,22,30,14,30,14,20,22,14,22,22].map(wch => ({ wch }));
    const xlWb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(xlWb, xlWs, "OD Operation");
